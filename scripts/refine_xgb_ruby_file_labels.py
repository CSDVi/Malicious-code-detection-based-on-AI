"""Downgrade repository-only Ruby positives without deleting any rows.

Ruby repositories frequently contain login configuration, UI, documentation,
and generic server helpers alongside the actual payload. This migration keeps
every row and original label metadata, but marks files without a strong
standalone behavior chain as needing review so they cannot influence
supervised training or held-out metrics.
"""
...
def refine(base: Path, output: Path, report_path: Path) -> dict[str, Any]:
    ...
    with base.open(encoding="utf-8") as source, temporary.open(
        ...
        for line in source:
            ...
            rows += 1
            if (
                str(row.get("language") or "").lower() == "ruby"
                and row.get("label") == "malicious"
                and row.get("source") == "github_ruby_malicious_candidates"
            ):
                code = str(row.get("code") or "")
                broad_count = _vx_signal_score(code)
                strong_count = ruby_high_confidence_behavior_count(code)
                if broad_count < 3 and strong_count == 0:
                    changed += 1
                    ...
                    row["label_basis"] = (
                        ...
                        "insufficient standalone Ruby file-local evidence; "
                        "retained for review and excluded from supervised metrics."
                    )
                    labels = list(row.get("behavior_labels") or [])
                    labels.extend([
                        f"file_local_signal_groups:{broad_count}",
                        f"ruby_high_confidence_behavior_groups:{strong_count}",
                        "weak_repository_only_positive",
                    ])
                    row["behavior_labels"] = sorted(set(map(str, labels)))
                    note = str(row.get("review_notes") or "").strip()
                    migration_note = (
                        "v46 Ruby label refinement: row preserved, original "
    ...
    report = {
        "schema_version": 1,
        "objective": "strict file-local Ruby malicious-label refinement",
        ...
        "selection_rule": (
            "github Ruby malicious candidate requires >=3 broad file-local "
