...
const DEFAULTS = Object.freeze({
  ...
  targetFps: 30,
  isBounce: false,
  colors: ["#2f1b06", "#7b4613", "#bd7c29", "#f0ca71"],
  autoDemo: true,
  autoSpeed: 0.28,
  autoIntensity: 1.8,
...
function mountLiquidEther(container, options = {}) {
  if (!container) return () => {};
...
class CommonClass {
  ...
  init(container) {
    ...
    // The fluid simulation already supplies its own soft edges. Extra DPR and
    // MSAA only make the full-screen composite more expensive without adding
    // visible detail behind the login card.
...
class MouseClass {
  ...
  onDocumentMouseMove(event) {
    ...
    if (now - this.lastPointerUpdate < this.pointerInterval) return;
...
class WebGLManager {
  constructor(props) {
    ...
    Mouse.pointerInterval = 1000 / Math.max(1, props.targetFps);
...
const io = new IntersectionObserver(
  entries => {
    ...
    const entryVisible = entry.isIntersecting && entry.intersectionRatio > 0;
    isVisible = entryVisible;
    if (!webglInstance) return;
    if (entryVisible && !document.hidden) {
      webglInstance.start();
    } else {
      webglInstance.pause();
    }
...
function initializeAuthLiquidEther() {
  ...
  const veryConstrainedDevice = memory <= 2 || cores <= 2;
  const constrainedDevice = saveData || memory <= 4 || cores <= 4;
  const highPixelLoad = window.innerWidth * window.innerHeight * Math.min(window.devicePixelRatio || 1, 2) ** 2 > 3_000_000;
  ...
  containers.forEach(container => {
    ...
    try {
      const quality = veryConstrainedDevice ? 'low' : constrainedDevice || highPixelLoad ? 'balanced' : 'standard';
      const targetFps = veryConstrainedDevice ? 20 : constrainedDevice ? 24 : 30;
      const resolution = veryConstrainedDevice ? 0.16 : constrainedDevice ? 0.2 : highPixelLoad ? 0.22 : 0.26;
      const iterationsPoisson = veryConstrainedDevice ? 7 : constrainedDevice ? 9 : 12;
      container.dataset.liquidQuality = quality;
      container.dataset.liquidFps = String(targetFps);
      const cleanup = mountLiquidEther(container, {
        resolution,
        iterationsPoisson,
        targetFps,
        BFECC: false
      ...
      container.classList.add("is-static");
      console.warn("Liquid Ether background unavailable.", error);
    }
  });
}

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initializeAuthLiquidEther, { once: true });
} else {
  initializeAuthLiquidEther();
}
